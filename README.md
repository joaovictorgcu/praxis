...
Console do banco em `/h2-console` (JDBC `jdbc:h2:mem:praxis`, usuário `sa`, sem senha). Desligar a carga de exemplo: `praxis.dados-exemplo=false`.
...
domain/         processo, prazo, documento, modelo, anexo, feriado, notificacao, honorario, usuario — Java puro
infrastructure/ persistence (JPA + mappers + adapters), notificacao, scheduler, seguranca (PBKDF2), config
```

A regra de dependência é fácil de enunciar e de verificar: **nada no `domain` importa Spring ou JPA**. Na prática, a regra do art. 219 do CPC é testável sem subir Spring, sem banco e sem HTTP.

Oito padrões de projeto, cada um por um problema real — Strategy (regimes de contagem e de honorário), Composite (dias não úteis), Template Method (esqueleto da peça), Interpreter (texto do modelo), Observer (andamento e prazo em risco), Decorator (canais de notificação), Proxy (segredo de justiça) e Iterator (linha do tempo). Quem faz o quê, e por quê: [`docs/arquitetura.md`](docs/arquitetura.md).
...
Os cenários são escritos em português, em [`src/test/resources/features`](src/test/resources/features), e exercitam os casos de uso reais contra o banco, não dublês. Os testes HTTP (MockMvc) fecham o que o BDD não vê: corpo da requisição, status de erro e as garantias de front e de sessão.
